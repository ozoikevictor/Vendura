...
describe("Vendura API", () => {
  it("creates a protected support request with a reference", async () => {
    const response = await request(app).post("/api/support/requests").send({
      ...
      category: "order",
      subject: "Missing delivery update",
      message: "My order has not received a delivery update.",
    });
    expect(response.status).toBe(201);
    expect(response.body.data.reference).toMatch(/^VEN-/);
    expect(
      (await db.list("supportRequests")).filter(
        (item) => item.kind === "support_request",
      ),
    ).toHaveLength(1);
  });
  ...
  it("registers, logs in, and returns the current user", async () => {
    ...
    const response = await request(app)
      .get("/api/auth/me")
      .set(auth(registration.body.data.token));
  ...
  it("marks registrations verified and stores no OTP while verification is disabled", async () => {
    ...
    }>("users", { email: "secure@example.com" });
    expect(registration.status).toBe(201);
  ...
  it("changes a signed-in user's password after checking the current password", async () => {
    ...
    expect(
      (
        await request(app)
          .post("/api/auth/login")
          .send({ email: "customer@vendura.test", password: "NewPassword456!" })
      ).status,
    ).toBe(200);
  });
  it("rejects invalid login details", async () => {
    expect(
      (
        await request(app)
          .post("/api/auth/login")
          .send({ email: "customer@vendura.test", password: "wrong" })
      ).status,
    ).toBe(401);
  });
  it("lists, filters, and resolves catalog records", async () => {
    expect((await request(app).get("/api/categories")).body.data).toHaveLength(
      3,
    );
    expect(
      (await request(app).get("/api/stores/slug/technaija")).body.data.id,
    ).toBe("store-technaija");
    const products = await request(app).get(
      "/api/products?q=aurora&inStockOnly=true",
    );
    expect(products.body.data.total).toBe(1);
    expect(
      (await request(app).get("/api/products/slug/aurora-5g-smartphone"))
        .status,
    ).toBe(200);
  });
  it("protects vendor routes by role", async () => {
    const customer = await login("customer@vendura.test");
    expect(
      (await request(app).get("/api/vendor/products").set(auth(customer)))
        .status,
    ).toBe(403);
  });
  it("allows a vendor to create, edit, duplicate, and delete a product", async () => {
    ...
    const created = await request(app)
      ...
      .send({
        name: "Test Laptop",
        description: "A complete test laptop",
        images: [],
        price: 500000,
  ...
  it("manages customer addresses", async () => {
    ...
    expect(created.status).toBe(201);
    expect(
      (await request(app).get("/api/users/me/addresses").set(auth(token))).body
        .data,
    ).toHaveLength(1);
  });
  it("accepts one verified review per delivered order product and updates ratings", async () => {
    await db.create("orders", {
      ...
      status: "delivered",
      items: [{ id: "item-review-1", productId: "product-phone-1" }],
    });
    const customer = await login("customer@vendura.test");
    const review = await request(app)
      .post("/api/products/product-phone-1/reviews")
      .set(auth(customer))
      .send({
        orderId: "order-review-1",
        rating: 5,
        comment: "Excellent phone and fast delivery.",
      });
    ...
    expect(
      (await request(app).get("/api/products/product-phone-1/reviews")).body
        .data,
    ).toHaveLength(1);
    expect(
      (
    ...
    expect(
      (
        await db.get<{ id: string; rating: number; reviewCount: number }>(
          "products",
          "product-phone-1",
        )
      )?.reviewCount,
  ...
  it("places an order using authoritative prices and delivery fees", async () => {
    const customer = await login("customer@vendura.test");
    const address = {
      fullName: "Demo Customer",
      phone: "+2348000000001",
    ...
    const placed = await request(app)
      ...
      .set(auth(customer))
      .send({
        items: [{ productId: "product-phone-1", quantity: 2, unitPrice: 1 }],
    ...
    const express = await request(app)
      .post("/api/orders")
      .set(auth(customer))
      .send({
        items: [{ productId: "product-phone-1", quantity: 1 }],
    ...
    expect(express.body.data).toMatchObject({
      subtotal: 245000,
      deliveryFee: 5000,
      total: 250000,
    });
    ...
    const updated = await request(app)
      ...
      .set(auth(vendor))
      .send({ type: "confirm" });
    expect(updated.body.data.status).toBe("payment_confirmed");
  });
  it("notifies the seller about a new order and the customer about status changes", async () => {
    ...
    expect(sellerNotifications.body.data[0]).toMatchObject({
      type: "new_order",
      read: false,
      href: `/vendor/orders/${placed.body.data.id}`,
    });
    ...
    expect(
      (await request(app).get("/api/notifications").set(auth(vendor))).body
        .data[0].read,
    ).toBe(true);
    ...
    await request(app)
      ...
      .set(auth(vendor))
      .send({ type: "confirm" });
    const customerNotifications = await request(app)
      .get("/api/notifications")
      .set(auth(customer));
    expect(customerNotifications.body.data[0]).toMatchObject({
      type: "order_confirmed",
      read: false,
      href: `/customer/orders/${placed.body.data.id}`,
    });
    expect(
      (
        await request(app)
    ...
    expect(
      (await request(app).get("/api/notifications").set(auth(customer))).body
        .data[0].read,
    ).toBe(true);
  });
  it("initializes and verifies a Paystack payment on the backend", async () => {
    ...
    try {
      ...
      const initialized = await request(app)
        ...
        .set(auth(customer))
        .send({ orderIds: [placed.body.data.id] });
      expect(initialized.status).toBe(200);
      expect(initialized.body.data.authorizationUrl).toBe(
      ...
      expect(verified.body.data.orders[0]).toMatchObject({
        paymentStatus: "paid",
      ...
      const vendor = await login("vendor@vendura.test");
      const notifications = (
        await request(app).get("/api/notifications").set(auth(vendor))
      ).body.data;
      expect(
        notifications.some(
          (item: { type: string }) => item.type === "payment_received",
        ),
      ).toBe(true);
      ...
      expect(pendingBalance).toMatchObject({
        available: 0,
        pending: 235250,
        customerPayments: 247500,
        totalSales: 245000,
        deliveryFees: 2500,
        platformFees: 12250,
      });
      ...
      expect(
        (await request(app).get("/api/vendor/balance").set(auth(vendor))).body
          .data.available,
      ).toBe(0);
      expect(
        (
      ...
      const event = {
        ...
        data: {
          reference: payout.body.data.reference,
          amount: 23525000,
          currency: "NGN",
          transferred_at: new Date().toISOString(),
        },
      };
      const signature = createHmac("sha512", config.PAYSTACK_SECRET_KEY!)
      ...
      expect(
        (await request(app).get("/api/vendor/payouts").set(auth(vendor))).body
          .data[0].status,
      ).toBe("paid");
    } finally {
  ...
  it("removes sold-out products from public listings and restores them after cancellation", async () => {
    await db.update("products", "product-phone-1", {
      stock: 1,
      status: "active",
    });
    const customer = await login("customer@vendura.test");
    ...
    expect(placed.status).toBe(201);
    expect(await db.get("products", "product-phone-1")).toMatchObject({
      stock: 0,
      status: "out_of_stock",
    });
    expect((await request(app).get("/api/products")).body.data.total).toBe(0);
    expect(
      (await request(app).get("/api/storefronts/technaija")).body.data,
    ).toMatchObject({
    ...
    const vendor = await login("vendor@vendura.test");
    expect(
      (await request(app).get("/api/vendor/overview").set(auth(vendor))).body
        .data.productsCount,
    ).toBe(0);
    expect(
  ...
  it("connects customer and vendor messages in one conversation", async () => {
    ...
    const conversation = await request(app)
      ...
      .set(auth(customer))
      .send({ productId: "product-phone-1" });
    ...
    expect(
      (
        await request(app)
          ...
          .set(auth(customer))
          .send({ text: "Is this available?" })
      ).status,
    ).toBe(201);
    const vendorInbox = await request(app)
      .get("/api/conversations")
      .set(auth(vendor));
    expect(vendorInbox.body.data[0]).toMatchObject({
      id: conversationId,
      lastMessage: "Is this available?",
      unreadForVendor: 1,
    });
    const vendorNotifications = await request(app)
      .get("/api/notifications")
      .set(auth(vendor));
    ...
    const messages = await request(app)
      ...
      .set(auth(customer));
    expect(
      messages.body.data.map((message: { text: string }) => message.text),
    ).toEqual(["Is this available?", "Yes, it is available."]);
    expect(
      (
        await request(app)
          .post(`/api/conversations/${conversationId}/offers`)
          .set(auth(customer))
          .send({ offeredPrice: 220000 })
      ).status,
    ).toBe(201);
    ...
    expect(
      (await request(app).get("/api/conversations").set(auth(customer))).body
        .data,
    ).toHaveLength(0);
    expect(
      (await request(app).get("/api/conversations").set(auth(vendor))).body
        .data,
    ).toHaveLength(1);
    await request(app)
      .post(`/api/conversations/${conversationId}/messages`)
      .set(auth(vendor))
      .send({ text: "I have sent an update." });
    expect(
      (await request(app).get("/api/conversations").set(auth(customer))).body
        .data[0].id,
    ).toBe(conversationId);
  ...
  it("sorts order history newest first and hides it separately for each account", async () => {
    ...
    await db.update("orders", older.body.data.id, {
      placedAt: "2026-01-01T10:00:00.000Z",
    });
    await db.update("orders", newer.body.data.id, {
      placedAt: "2026-01-02T10:00:00.000Z",
    });

    expect(
      (await request(app).get("/api/orders").set(auth(customer))).body.data.map(
        (order: { id: string }) => order.id,
      ),
    ).toEqual([newer.body.data.id, older.body.data.id]);
    expect(
      (
        await request(app).get("/api/vendor/orders").set(auth(vendor))
      ).body.data.map((order: { id: string }) => order.id),
    ).toEqual([newer.body.data.id, older.body.data.id]);
    expect(
      (
        await request(app)
          .delete(`/api/orders/${newer.body.data.id}`)
          .set(auth(customer))
      ).status,
    ).toBe(204);
    expect(
    ...
    ).toEqual([older.body.data.id]);
    expect(
      (await request(app).get("/api/vendor/orders").set(auth(vendor))).body
        .data,
    ).toHaveLength(2);
    expect(
      (
  ...
  it("derives public store product counts from live active listings", async () => {
    ...
    expect(storefront.body.data.store.productCount).toBe(
      storefront.body.data.products.length,
    );
    expect(storefront.body.data.store.productCount).toBe(1);
    const stores = await request(app).get("/api/stores");
    expect(
      stores.body.data.find(
        (store: { id: string }) => store.id === "store-technaija",
      ).productCount,
    ).toBe(1);
  });
  it("searches live products through the authenticated shopping assistant", async () => {
    ...
    const generic = await request(app)
      .post("/api/ai/search")
      .set(auth(customer))
      .send({ message: "I need a phone under ₦300,000" });
    expect(generic.body.data.products[0]).toMatchObject({
      id: "product-phone-1",
    ...
    const specific = await request(app)
      .post("/api/ai/search")
      .set(auth(customer))
      .send({ message: "I need a red phone" });
    expect(specific.body.data.products).toHaveLength(0);
    const buyerRequest = await request(app)
    ...
    expect(buyerRequest.body.data).toMatchObject({
      product: "Red leather bag",
      status: "active",
      offerCount: 0,
    });
    const requests = await request(app)
      .get("/api/ai/requests")
      .set(auth(customer));
    expect(requests.body.data[0]).toMatchObject({
      id: buyerRequest.body.data.id,
      maximumBudget: 50000,
    });
    ...
    const count = await request(app)
      .post("/api/ai/search")
      .set(auth(customer))
      .send({ message: "How many products are on Vendura?" });
    ...
    expect(availability.body.data.products[0]).toMatchObject({
      id: "product-phone-1",
    });
    expect(availability.body.data.response).toContain(
      "Yes. I found 1 matching in-stock product",
    );
    const bareProduct = await request(app)
      .post("/api/ai/search")
      .set(auth(customer))
      .send({ message: "phone" });
    expect(bareProduct.body.data).toMatchObject({ intent: "shopping" });
    expect(bareProduct.body.data.products[0]).toMatchObject({
      id: "product-phone-1",
    });
    const colours = await request(app)
      .post("/api/ai/search")
      .set(auth(customer))
      .send({
    ...
    expect(colours.body.data.products[0]).toMatchObject({
      id: "product-phone-1",
    });
    expect(colours.body.data.response).toContain(
      "have not listed colour options yet",
    );
    const aiOffers = await request(app)
      .get("/api/ai/offers")
      .set(auth(customer));
  ...
  it("returns vendor dashboard, AI, settings, subscription, and finance data", async () => {
    ...
    expect(overview.body.data.revenueSeries).toHaveLength(7);
    expect(overview.body.data.ordersSeries).toHaveLength(7);
    const ai = await request(app)
      .post("/api/vendor/ai/search")
      .set(auth(token))
      .send({ message: "How many products do I have?" });
    expect(ai.status).toBe(200);
    expect(ai.body.data.response).toContain("1 product listing");
    expect(ai.body.data.metrics).toEqual(
      expect.arrayContaining([{ label: "All products", value: "1" }]),
    );
    expect(
      (await request(app).get("/api/vendor/delivery-settings").set(auth(token)))
        .body.data.pickupAvailable,
    ).toBe(true);
    expect(
      (await request(app).get("/api/vendor/subscription").set(auth(token))).body
        .data.planId,
    ).toBe("growth");
    expect((await request(app).get("/api/plans")).body.data).toHaveLength(3);
  });
  ...
  it("enforces monthly subscription status and plan product limits", async () => {
    ...
    const atLimit = await request(app)
      ...
      .send({
        name: "One too many",
        description: "This product exceeds the starter limit.",
        images: [],
        price: 5000,
        categoryId: "cat-electronics",
        sku: "LIMIT-21",
    ...
    const expired = await request(app)
      ...
      .set(auth(token))
      .send({
        name: "Expired plan product",
        description: "This product requires a renewed plan.",
        images: [],
        price: 5000,
        categoryId: "cat-electronics",
        sku: "EXPIRED-1",
    ...
    expect(
      (await request(app).get("/api/vendor/subscription").set(auth(token))).body
        .data,
    ).toMatchObject({ status: "past_due", isActive: false });
  });
  it("activates a paid monthly subscription through Paystack", async () => {
    ...
    try {
      ...
      const initialized = await request(app)
        .post("/api/vendor/subscription/paystack/initialize")
        .set(auth(token))
        .send({ planId: "starter" });
      expect(initialized.status).toBe(200);
      expect(initialized.body.data.authorizationUrl).toBe(
        "https://checkout.paystack.com/subscription",
      );
      const reference = initialized.body.data.reference as string;
      const verified = await request(app)
        .get(`/api/vendor/subscription/paystack/verify/${reference}`)
        .set(auth(token));
      expect(verified.status).toBe(200);
      ...
      expect(
        new Date(verified.body.data.currentPeriodEnd).getTime(),
      ).toBeGreaterThan(
  ...
  it("answers live vendor business questions and ranks newest orders first", async () => {
    ...
    await db.create("orders", {
      ...
      total: 10000,
      status: "placed",
      placedAt: "2026-01-01T08:00:00.000Z",
    });
    await db.create("orders", {
      id: "order-new",
      orderNumber: "VND-NEW",
      ...
      total: 20000,
      status: "placed",
      placedAt: "2026-09-24T08:00:00.000Z",
    });
    await db.update("products", "product-phone-1", {
      stock: 0,
      status: "out_of_stock",
    ...
    const orders = await request(app)
      ...
      .set(auth(token));
    expect(orders.body.data.map((order: { id: string }) => order.id)).toEqual([
      "order-new",
      "order-old",
    ]);
    const latest = await request(app)
      .post("/api/vendor/ai/search")
      .set(auth(token))
      .send({ message: "What is my newest order?" });
    expect(latest.body.data.response).toContain("VND-NEW");
    expect(latest.body.data.items[0]).toMatchObject({
      id: "order-new",
      type: "order",
    });
    const stock = await request(app)
      .post("/api/vendor/ai/search")
      .set(auth(token))
      .send({ message: "Which products are out of stock?" });
    expect(stock.body.data.items[0]).toMatchObject({
      id: "product-phone-1",
      type: "product",
    });
    const payout = await request(app)
      .post("/api/vendor/ai/search")
      .set(auth(token))
      .send({ message: "Show my payouts for today" });
    expect(payout.body.data.metrics).toEqual(
      expect.arrayContaining([{ label: "Amount", value: "₦5,000" }]),
    );
  });
  it("lists banks and verifies a seller bank account through Paystack", async () => {
      ...
      if (url.includes("/bank?"))
        return new Response(
          JSON.stringify({
            status: true,
            message: "Banks",
            data: [{ id: 1, name: "Test Bank", code: "999", active: true }],
          }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
    ...
    try {
      const vendor = await login("vendor@vendura.test");
      expect(
        (await request(app).get("/api/vendor/banks").set(auth(vendor))).body
          .data[0],
      ).toMatchObject({ name: "Test Bank", code: "999" });
      const saved = await request(app)
        .put("/api/vendor/bank-account")
        .set(auth(vendor))
        .send({ bankCode: "999", accountNumber: "0123456789" });
      expect(saved.status).toBe(200);
      expect(saved.body.data).toMatchObject({
        bankName: "Test Bank",
        bankCode: "999",
        accountName: "VICTOR JAMES",
  ...
  it("rejects unsigned payout webhooks and restores a failed payout once", async () => {
    ...
    try {
      await db.create("transactions", {
        id: "payout-ledger-test",
        vendorId: "user-vendor-1",
        type: "payout",
        amount: -1000,
        status: "available",
        reference: "vendura-failed-transfer-123",
        description: "Test payout",
        createdAt: new Date().toISOString(),
      });
      await db.create("payouts", {
        id: "payout-failed-test",
        vendorId: "user-vendor-1",
        amount: 1000,
        status: "processing",
        reference: "vendura-failed-transfer-123",
      ...
      expect(
        (await request(app).post("/api/webhooks/paystack").send(event)).status,
      ).toBe(401);
      const signature = createHmac("sha512", config.PAYSTACK_SECRET_KEY)
        .update(JSON.stringify(event))
        .digest("hex");
      ...
      expect((await db.get("payouts", "payout-failed-test"))?.status).toBe(
        "failed",
      );
      expect((await db.get("transactions", "payout-ledger-test"))?.status).toBe(
        "reversed",
      );
      const notifications = await db.list<{ id: string; type: string }>(
        "notifications",
      );
      expect(
        notifications.filter((item) => item.type === "payout_processed"),
      ).toHaveLength(1);
      expect(
        (
          await request(app)
      ...
      expect(
        (await db.list<{ id: string; type: string }>("notifications")).filter(
          (item) => item.type === "payout_processed",
        ),
      ).toHaveLength(1);
    } finally {
      config.PAYSTACK_SECRET_KEY = originalKey;
  ...
  it("protects the admin console and lets an admin manage accounts and stores", async () => {
    const customer = await login("customer@vendura.test");
    expect(
      (await request(app).get("/api/admin/overview").set(auth(customer)))
        .status,
    ).toBe(403);
    ...
    expect(
      (
        await request(app)
          .post("/api/auth/login")
          .send({ email: "customer@vendura.test", password: "Password123!" })
      ).status,
    ).toBe(403);
  ...
  it("supports the complete seller storefront and customer order journey", async () => {
    ...
    const sellerSubscription = await db.findOne("subscriptions", {
      vendorId: seller.body.data.user.id,
    });
    expect(sellerSubscription).toBeDefined();
    await db.update("subscriptions", sellerSubscription!.id, {
      status: "active",
