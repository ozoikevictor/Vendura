...
export const messageRoutes = (db: Database) => {
  ...
  router.post("/conversations/:id/messages", asyncRoute(async (req: AuthRequest, res) => {
    ...
    await db.update("conversations", conversation.id, { lastMessage: text, lastMessageAt: message.sentAt, [role === "customer" ? "unreadForVendor" : "unreadForCustomer"]: Number(conversation[role === "customer" ? "unreadForVendor" : "unreadForCustomer"]) + 1, [role === "customer" ? "hiddenForVendor" : "hiddenForCustomer"]: false });
  ...
  router.delete("/conversations/:id", asyncRoute(async (req: AuthRequest, res) => { const conversation = await accessibleConversation(db, req.params.id, req); const role = conversation.customerId === req.user!.id ? "customer" : "vendor"; await db.update("conversations", conversation.id, { [role === "customer" ? "hiddenForCustomer" : "hiddenForVendor"]: true }); res.status(204).end(); }));
