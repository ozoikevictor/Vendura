    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
        <div className="rounded-xl border border-primary/30 bg-primary-soft p-4">
          ...
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.available)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.pending)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">Customer Paid</div>
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.customerPayments)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">Total Sales</div>
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.totalSales)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">Delivery Collected</div>
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.deliveryFees)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground">Vendura Fee (5%)</div>
          <p className="mt-1 text-xl font-bold text-foreground">{formatNaira(balance.platformFees)}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">{bank ? <>{bank.accountNumber} · {bank.accountName} {bank.verified ? <span className="text-success">✓ Verified</span> : <span className="text-warning-foreground">Awaiting verification</span>}</> : "Add your Nigerian bank account to receive payouts."}</p>
        </div>
        <button onClick={() => { if (bank) setBankForm({ bankCode: bank.bankCode ?? "", accountNumber: bank.accountNumber }); setShowBank((v) => !v); }} className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-2 text-sm font-semibold text-foreground hover:bg-accent">
          <Building2 className="h-4 w-4" /> {bank ? "Edit Bank" : "Add Bank"}
        </button>
        <button onClick={handleRequestPayout} disabled={requesting || !bank?.verified || balance.available <= 0} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {showBank && (
        <div className="space-y-3 rounded-xl border border-border bg-card p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <select value={bankForm.bankCode} onChange={(e) => setBankForm((f) => ({ ...f, bankCode: e.target.value }))} disabled={banksLoading} className="rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary">
              <option value="">{banksLoading ? "Loading banks..." : "Select bank"}</option>
              {banks.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}
            </select>
            <input value={bankForm.accountNumber} onChange={(e) => setBankForm((f) => ({ ...f, accountNumber: e.target.value.replace(/\D/g, "").slice(0, 10) }))} placeholder="10-digit account number" inputMode="numeric" className="rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
          </div>
          ...
          {txns?.map((t) => (
            <div key={t.id} className="flex items-center justify-between py-2.5">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{formatDateTime(t.createdAt)} · {t.reference}</p>
              </div>
            ...
            <div key={p.id} className="flex items-center justify-between py-2.5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{p.reference} · {formatDateTime(p.requestedAt)}</p>
                </div>
              </div>
