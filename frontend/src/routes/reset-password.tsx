    ...
    <div className="flex min-h-screen items-center justify-center lagoon-wash px-4 py-12">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">Enter the reset code from your email and set a new password.</p>
        </div>

        <div className="rounded-2xl border border-border bg-card p-6 shadow-card sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="token" className="mb-1.5 block text-sm font-medium text-foreground">Reset code</label>
              <input id="token" type="text" required value={token} onChange={(e) => setToken(e.target.value)} placeholder="Enter code from email"
                className="w-full rounded-lg border border-input bg-background py-2.5 px-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
            <div>
