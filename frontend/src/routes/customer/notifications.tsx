...
function CustomerNotificationsPage() {
  ...
  return <div className="min-h-screen lagoon-wash"><MarketplaceHeader /><main className="mx-auto min-h-[60vh] max-w-4xl px-4 py-8 sm:px-6">
    ...
    {isLoading ? <div className="space-y-2">{[1, 2, 3].map((i) => <div key={i} className="h-20 animate-pulse rounded-lg bg-muted" />)}</div> : notifications.length > 0 ? <NotificationList notifications={notifications} onOpen={openNotification} /> : <EmptyState icon={<Bell className="h-7 w-7" />} title="No notifications" description="Updates about your orders will appear here." />}
  </main><SiteFooter /></div>;
}
