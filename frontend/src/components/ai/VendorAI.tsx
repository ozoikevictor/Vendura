    ...
    <div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden rounded-lg border border-border bg-card">
      ...
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); send(); }} className="shrink-0 border-t border-border bg-card p-2 sm:p-4">
        <div className="mx-auto flex max-w-3xl items-end gap-2 rounded-lg border border-input bg-background p-2">
          <textarea ref={composerInput} value={input} onPointerDown={(event) => { if (document.activeElement !== composerInput.current) { event.preventDefault(); composerInput.current?.focus({ preventScroll: true }); } }} onChange={(event) => { setInput(event.target.value); if (error) setError(""); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} rows={1} placeholder="Ask about your store..." className="max-h-28 min-h-10 min-w-0 flex-1 resize-none overflow-y-auto bg-transparent px-2 py-2 text-base outline-none sm:text-sm" />
